...
export default function TeamCard({ name, role, bio, photo }) {
  return (
    <figure className="w-80 h-80 px-6 py-8 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-md flex flex-col items-center">
      <Image
        src={photo}
        alt={name}
        width={128}
        height={128}
        className="h-32 w-32 rounded-full object-cover mb-4 flex-shrink-0"
        loading="lazy"
      />
